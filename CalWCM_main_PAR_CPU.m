...
debug = 0;  %0/1 to check and get some display
...
experiment      = 'EXP'; %'po_ol_hymap_noirr';
path_LIS_output = '/staging/leuven/stg_00024/OUTPUT/projects/4DMED/output_2015-2023_v5/';
...
if (PAR_CPU > PAR_N_CPU)
    error('Processor number cannot exceed number of processors')
end
path_cal_output = [path_cal_output,'/',num2str(PAR_CPU),'_',num2str(PAR_N_CPU),'/'];
if ~exist(path_cal_output, 'dir')
     mkdir(path_cal_output);
end
%===
...
if ~exist(file_out_mat,'file')
    ...
    [DD, lat_all, lon_all, lat, lon, domain, LIS_ssm, LIS_veg, LIS_obs]...
        ...
        min_lat, max_lat, min_lon, max_lon, '.a01', 1, model, PAR_CPU, PAR_N_CPU);
    save (file_out_mat,...
...
for  i= 1:length(domain) 
    ...
    if (isempty(fwd_in.sm) || isempty(fwd_in.veg) || isempty(obs) ||...
       ...
       disp(['NOT calibrating lat-lon: ', num2str(lat(i)),'-', num2str(lon(i)),' (',num2str(progr),')']);
       
    else
       disp(['Calibrating lat-lon: ', num2str(lat(i)),'-', num2str(lon(i)),' (',num2str(progr),')']);
